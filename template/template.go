...
func (t *Template) Execute(w io.Writer, v Values) error {
	...
	for i, m := range collated {
		if m.Role == "user" {
			value.Prompt = m.Content
		} else {
			value.Response = m.Content
		}

		if i != len(collated)-1 && value.Prompt != "" && value.Response != "" {
			if err := t.Template.Execute(&b, value); err != nil {
				return err
			}
...
func collate(msgs []api.Message) (system string, collated []*api.Message) {
	...
	for _, m := range msgs {
		...
		for range m.Images {
			imageTag := fmt.Sprintf("[img-%d]", i)
			if !strings.Contains(m.Content, "[img]") {
				m.Content = strings.TrimSpace("[img] " + m.Content)
			}

			m.Content = strings.Replace(m.Content, "[img]", imageTag, 1)
			i++
		}

		if len(collated) > 0 && collated[len(collated)-1].Role == m.Role {
			collated[len(collated)-1].Content += "\n\n" + m.Content
		} else {
			collated = append(collated, &m)
		}
	}
